...
namespace espurna
{
    namespace web
    {
        namespace ws
        {
            ...
            EnumerableConfig::EnumerableConfig(JsonObject &root, StringView name) : _root(root.createNestedObject(FPSTR(name.c_str())))
            {
            }
            ...
            EnumerablePayload::EnumerablePayload(JsonObject &root, StringView name) : _root(root.createNestedObject(FPSTR(name.c_str())))
            {
            }
...
namespace
{
    ...
    void _onAuth(AsyncWebServerRequest *request)
    {
        ...
        {
            if (!(*it).ip.isSet() || ((*it).ip == ip) || (now - (*it).timestamp > WsTimeout))
            {
                break;
            }
            ++it;
...
namespace
{
    ...
    {
        return (key == STRING_VIEW("adminPass")) || (key == STRING_VIEW("hostname")) || (key == STRING_VIEW("desc")) || (key == STRING_VIEW("webPort")) || key.startsWith(STRING_VIEW("ws"));
    }
